...
#' Read in the data. 
#------------------------------------------------------------------------#
cap_data <- read.csv("BabbageRiver_capture_full.csv")
dd_fl <- read.csv("BabbageRiver_forklength_full.csv")
...
code_m <- nimbleCode({
  ...
  alpha[1] ~ dnorm(0,sd = 100)
  alpha[2] ~ dnorm(0,sd = 100)
  ...
  for (t in 1:nyears){
    ...
    mu[t] ~ dgamma(shape = mu_o, scale = sigma_mu_o)
  }
  ...
  for (i in 1:N) {
    ...
    for (t in (first[i]+1):nyears) {
      ...
      fl[i, t] ~ T(dnorm(fl[i, t-1] + mu[t], sd = sigma), fl[i,t-1], )
...
mu_inits <- runif(nyears)
#head(mu_inits)

#------------------------ Final initialization --------------------------#

initial.values <- list(alpha = runif(2, 0, 1), beta = runif(3,0,1),
                       mu_o = 0.001, sigma_mu_o = 0.1,
                       sigma = 1,
                       a_sigma = 0.001, b_sigma = 100,
                       fl = fl_inits, z = z_inits, mu = mu_inits)
...
pdf(file = "Bab_fl_mp_Q_m1.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5)
MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Bab_fl_Q_m1", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Bab_fl_m1_Q_sum.csv")