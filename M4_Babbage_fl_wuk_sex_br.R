...
cap_data <- read.csv("BabbageRiver_capture_full_wuk_sex.csv")
dd_fl <- read.csv("BabbageRiver_forklength_full.csv")
b_fl <- read.csv("BabbageRiver_breeding_full_wuk_sex.csv")
...
code_m <- nimbleCode({
  ...
  for (i in 1:N) {
    ...
    for (t in (first[i]+1):nyears) {
      ...
      b[i,t] ~ dbern((sex[i]*((psi[1]*b[i,t-1])+(psi[2]*(1-b[i,t-1]))))+
                       ((1-sex[i])*((psi[3]*b[i,t-1])+(psi[4]*(1-b[i,t-1])))))
...
my.data = list(y=y + 1,b = b_fl, fl = d_fl, sex = sex.st)
...
pdf(file = "Bab_fl_mp_m4.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5, params = c("alpha", "beta", "psi"))
MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Bab_fl_m4", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Bab_fl_m4_sum.csv")
