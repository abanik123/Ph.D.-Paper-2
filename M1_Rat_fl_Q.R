...
#' Read in the data. 
#------------------------------------------------------------------------#
cap_data <- read.csv("RatRiver_capture_full.csv")
dd_fl <- read.csv("RatRiver_forklength_full.csv")
...
pdf(file = "Rat_fl_mp_Q_m1.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5)
MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Rat_fl_Q_m1", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Rat_fl_m1_Q_sum.csv")