...
pdf(file = "Big_fl_mp_m4.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5, params = c("alpha", "beta", "psi"))
MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Big_fl_m4", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Big_fl_m4_sum.csv")
...
#compile_MCMC_m1 <- compileNimble(McMC_m1, project = mcmc.nim_model)
#samples_m1 <- runMCMC(compile_MCMC_m1, niter = n.iter, nchains = n.chains,
#                      nburnin = n.burnin,summary = TRUE,
#                      WAIC = TRUE)
#--- CHECKED NIMBLE MODEL ---#
