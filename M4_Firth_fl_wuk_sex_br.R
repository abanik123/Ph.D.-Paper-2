...
b_fl <- read.csv("FirthRiver_breeding_full_wuk_sex.csv")
...
pdf(file = "Firth_fl_mp_m4.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5, params = c("alpha", "beta", "psi"))
MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Firth_fl_m4", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Firth_fl_m4_sum.csv")