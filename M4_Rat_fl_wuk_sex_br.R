...
cap_data <- read.csv("RatRiver_capture_full_wuk_sex.csv")
dd_fl <- read.csv("RatRiver_forklength_full.csv")
b_fl <- read.csv("RatRiver_breeding_full_wuk_sex.csv")
...
pdf(file = "Rat_fl_mp_m4.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5, params = c("alpha", "beta", "psi"))
MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Rat_fl_m4", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Rat_fl_m4_sum.csv")