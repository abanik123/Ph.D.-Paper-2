...
#' Read in the data. 
#------------------------------------------------------------------------#
cap_data <- read.csv("FirthRiver_capture_full.csv")
...
pdf(file = "Firth_fl_mp_Q_m1.pdf")
MCMCplot(samples, HPD = T)
dev.off()

s <- MCMCsummary(samples, round = 5)
MCMCtrace(samples,pdf = T,open_pdf = F,filename = "Firth_fl_Q_m1", ind = TRUE,
          Rhat = FALSE, n.eff = FALSE)
write.csv(s, file = "Firth_fl_m1_Q_sum.csv")
